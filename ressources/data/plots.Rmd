---
title: "CPESR"
author: "CPESR"
date: "`r Sys.Date()`"
output: github_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
options(dplyr.summarise.inform = FALSE)
knitr::opts_chunk$set(fig.asp = 9/16, fig.retina = 2)

library(tidyverse)
library(ggcpesrthemes)
library(kpiESR)
theme_set(theme_cpesr())
theme_cpesr_setup(authors = "Julien Gossa", source = "https://github.com/cpesr/RFC/")

load("RD.RData")
```

## plot_SCSPvsEtu

```{r, results='asis'}
plot_SCSPvsEtu <- kpiesr_plot_line("Universités et assimilés", c("kpi.ETU.P.effectif","kpi.FIN.S.SCSP"), val="evolution") +
  ggtitle("Evolution du nombre d'étudiants et de la SCSP", subtitle="Subvention pour charge de service public - Périmètre Universités")
plot_SCSPvsEtu
```

## plot_RecettesFormation

```{r, results='asis'}
plot_RecettesFormation <- kpiesr_plot_line("Ensemble",c("kpi.ETU.P.effectif","kpi.FIN.S.recettesFormation","kpi.FIN.S.SCSP"), val="evolution") +
  ggtitle("Evolution des recettes formation vs. SCSP et nombre d'étudiants", subtitle="Droits d'inscription, DU, VAE, formation continue, taxe d'apprentissage") +
  guides(linetype="none",alpha="none",size="none")

plot_RecettesFormation

esr %>% 
  filter(Groupe == "Ensemble") %>%
  select(Rentrée, Groupe, kpi.FIN.S.recettesFormation, kpi.K.forPetu) %>%
  spoiler_table("RecettesFormation",trim = Inf)
```

## plot_RessourcesPropres

```{r, results='asis'}
plot_RessourcesPropres <- kpiesr_plot_line("Ensemble",c("kpi.ETU.P.effectif","kpi.FIN.S.recettesFormation","kpi.FIN.S.recettesRecherche","kpi.FIN.S.SCSP"), val="evolution") +
  ggtitle("Evolution des recettes propres vs. SCSP et nombre d'étudiants", subtitle="Droits d'inscription, DU, VAE, formation continue, taxe d'apprentissage / ANR, ERC, contrats de recherche") +
  guides(linetype="none",alpha="none",size="none")

plot_RessourcesPropres

esr %>% 
  filter(Groupe == "Ensemble") %>%
  select(Rentrée, Groupe, kpi.FIN.S.recettesFormation, kpi.K.forPetu, kpi.FIN.S.recettesRecherche, kpi.K.recPect) %>%
  spoiler_table("RecettesFormation",trim = Inf)
```

## plot_SCSPvsMS

```{r, results='asis'}
plot_SCSPvsMS <- kpiESR::esr %>%
  #filter(Etablissement == "Universités et assimilés", Rentrée > 2008) %>%
  filter(Etablissement == "Ensemble", Rentrée > 2008) %>%
  mutate(diff = kpi.FIN.S.SCSP-kpi.FIN.S.masseSalariale) %>% 
  ggplot_sd(aes(x=Rentrée+1,y=diff)) +
  geom_hline(yintercept = 1) +
  geom_line(size=1, color="darkorange") + geom_point(shape = 21, stroke=2,size=2,fill="white",color="darkorange") +
  scale_x_continuous(breaks = scales::pretty_breaks(12)) +
  scale_y_continuous(labels = ~ paste(.x/1e6,"M€"), name = "SCSP-MS") +
  scale_color_distiller(palette="Reds") +
  theme(legend.position = "None", panel.grid.minor.x = element_blank()) +
  labs(
    title="Différence entre SCSP et masse salariale des établissements du MESR",
    x="Année") 
plot_SCSPvsMS
```

```{r, results='asis'}
ylabeller <- function(.x) {
  case_when(
    .x <= 0 ~ paste(.x/1e6,"M€"),
    .x < 100 ~ paste(.x,"%"),
    .x < 10000 ~ paste(.x/1e3,"k€"),
    TRUE ~ paste(.x/1e6,"M€")
  )
}

plot_SCSPvsMS2 <- bind_rows(
  kpiESR::esr %>%
    filter(Etablissement == "Ensemble", Rentrée > 2008) %>%
    transmute(
      date = as.Date(paste0(Rentrée+1,"-01-01")),
      type = "SCSPvsMS",
      indicateur = "SCSPvsMS",
      valeur = kpi.FIN.S.SCSP-kpi.FIN.S.masseSalariale),
  read.csv("point-indice.csv") %>%
    transmute(
      date = as.Date(date, format = "%Y-%m-%d"),
      type = "Point d'indice",
      indicateur = "Point d'indice",
      valeur = montant_annuel_indice_100
    ),
  read.csv("cas-pension.csv") %>%
    transmute(
      date = as.Date(date, format = "%Y-%m-%d"),
      type = "Pensions",
      indicateur = "Taux CAS Pension",
      valeur = taux_cas_titulaire
    ),
  read.csv("cas-pension.csv") %>%
    transmute(
      date = as.Date(date, format = "%Y-%m-%d"),
      type = "Pensions",
      indicateur = "Taux cotisation pensions contractuels",
      valeur = taux_retraite_contractuel_tranche_A
    )
  ) %>%
  filter(date>=2010) %>%
  ggplot_sd(aes(x=date,y=valeur,color=indicateur)) +
  geom_line(size=1, ) + geom_point(shape = 21, stroke=1,size=1.5,fill="white") +
  #scale_x_continuous(breaks = scales::pretty_breaks(12)) +
  scale_y_continuous(labels = ~ ylabeller(.x)) +
  facet_wrap(type~., scales = "free") +
  expand_limits(y=0) +
  theme(legend.position = "None", panel.grid.minor.x = element_blank()) +
  labs(
    title="Différence entre SCSP et masse salariale des établissements du MESR\net cotisations pension + point d'indice",
    x="Année") 

plot_SCSPvsMS2
```


```{r, results='asis'}
evenements <- read.csv("evenements.csv") %>%
  mutate(date = as.Date(date))

plot_SCSPvsMS3 <- kpiESR::esr %>%
  #filter(Etablissement == "Universités et assimilés", Rentrée > 2008) %>%
  filter(Etablissement == "Ensemble", Rentrée > 2008) %>%
  mutate(diff = kpi.FIN.S.SCSP-kpi.FIN.S.masseSalariale) %>% 
  mutate(date = as.Date(paste0(Rentrée+1,"-01-01"))) %>%
  ggplot_sd(aes(x=date,y=diff)) +
  geom_hline(yintercept = 1) +
  geom_line(size=1, color="darkorange") + geom_point(shape = 21, stroke=2,size=2,fill="white",color="darkorange") +
  geom_text(data=evenements, aes(label=dispositif), y=-1e8, angle = 90, hjust=1, ) +
  # geom_text(data = tibble(année = 2010:2026, date = as.Date(paste0(année, "-01-01"))), 
  #           aes(label=année), y=3e7, vjust=0, size=3) +
  scale_x_date(date_breaks = "1 year",date_labels = "%Y") +
  scale_y_continuous(labels = ~ paste(.x/1e6,"M€"), name = "SCSP-MS") +
  scale_color_distiller(palette="Reds") +
  theme(legend.position = "None", panel.grid.minor.x = element_blank()) +
  labs(
    title="Différence entre SCSP et masse salariale des établissements du MESR",
    x="Année")

plot_SCSPvsMS3
```

## plot_depensesreformes

```{r, results='asis'}
plot_depensereformes <- depense_plot(c("CIR","Alternance","P150"))
plot_depensereformes
```

```{r, results='asis'}
plot_depensereformes_mesr <- depense_plot(c("CIR","Alternance","Établissements MESR"))
plot_depensereformes_mesr
```



## plot_MassificationRecent

```{r}
plot_MassificationRecent <- read.csv("kh-sl_eleves_sup.csv") %>%
  filter(champ_geo!="") %>%
  filter(annee >= 2000) %>%
  mutate(champ_geo = factor(champ_geo,levels=unique(champ_geo))) %>%
  filter(!is.na(disc_total)) %>%
  ggplot(aes(x=annee,y=disc_total, color = champ_geo)) +
  annotate("rect", xmin = 2000, xmax = 2021, ymin = 1.3e6, ymax = 1.7e6, color="black", fill=NA) +
  geom_line(size = 1) +
  scale_y_continuous(labels = ~ paste(.x/1e6,"M"), name = "Effectif") +
  scale_color_discrete(name = "Périmètre") +
  ggtitle("Evolution des effectifs étudiants du MESR") +
  theme(legend.position = "bottom") + 
  guides(color=guide_legend(nrow = 2)) +
  cpesr_cap(author = "Julien Gossa et Kévin Hédé", source="Données collectées par Kevin Hédé") 
plot_MassificationRecent
```

## plot_MassificationLong


```{r}
plot_MassificationLong <- read.csv("kh-sl_eleves_sup.csv") %>%
  filter(champ_geo!="") %>%
  mutate(champ_geo = factor(champ_geo,levels=rev(unique(champ_geo)))) %>%
  filter(!is.na(disc_total)) %>%
  ggplot(aes(x=annee,y=disc_total, color = champ_geo)) +
  annotate("rect", xmin = 2000, xmax = 2021, ymin = 1.3e6, ymax = 1.7e6, color="black", fill=NA) +
  geom_line(size = 1) +
  scale_y_continuous(labels = ~ paste(.x/1e6,"M"), name = "Effectif") +
  scale_color_discrete(name = "Périmètre") +
  ggtitle("Evolution des effectifs étudiants du MESR") +
  theme(legend.position = "bottom") + 
  guides(color=guide_legend(nrow = 2)) +
  cpesr_cap(author = "Julien Gossa et Kévin Hédé", source="Données collectées par Kevin Hédé") 
plot_MassificationLong
```

## plot_LvsM

```{r, fig.asp=7/16, fig.width=7, out.width="100%", fig.align='center'}
load(file="sise.RData")

plot_LvsM <- sise %>%
  group_by(Rentrée,Niveau = paste(LMD, substr(NIVEAU,2,2), sep="")) %>%
  summarise(Etudiants = sum(Etudiants)) %>%
  filter(Niveau %in% c("L3","M1")) %>%
  mutate(Rentrée = ifelse(Niveau == "L3", Rentrée+1,Rentrée)) %>%
  filter(Rentrée != 2012, Rentrée != 2023) %>%
  ggplot(aes(x=Rentrée, y= Etudiants, color=Niveau)) +
  geom_line(linewidth = 1.5) + geom_point(shape = 21, stroke = 2, size = 2, fill="white") + 
  scale_x_continuous(labels = ~ paste0("L3 ", .x-1,"\nM1 ",.x), breaks = seq(2011,2030,2), name="Cohorte") +
  scale_y_continuous(labels = ~ paste0(.x/1e3,"k"), name = "Inscriptions étudiantes") +
  ggtitle("Etudiant⋅e⋅s  inscrit⋅e⋅s en L3 à l'année N et M1 à l'année N+1" )

plot_LvsM
```

## plot_postesMCF

```{r, out.width="90%", fig.align="center"}
emplois.ec <- read.csv2("cpesr-emplois-ec.csv")

plot_postesMCF <- emplois.ec %>% 
  filter(Périmètre.ID %in% c("Ensemble","ST","LLASHS","DEG")) %>%
  mutate(Périmètre.label=factor(Périmètre.label, 
        levels=c("Ensemble","Sciences et techniques","Lettres et sciences humaines","Droit éco gestion"))) %>%
  mutate(Concours.NonPourvus.MCF = Concours.Postes.MCF - Concours.Recrutés.MCF) %>%
  ggplot(aes(x=Année, y=Concours.Postes.MCF, color=Périmètre.label)) +
  geom_line(linewidth = 1) + geom_point(shape = 21, stroke = 1.5, size = 1.5, fill="white") +
  scale_x_continuous(breaks = scales::pretty_breaks(12)) +
  labs(title="Concours MCF : Postes ouverts", 
       y = "Nombre de postes") 
plot_postesMCF
```

## plot_postesMCFvsCandidats

```{r, out.width="90%", fig.align="center"}
plot_postesMCFvsCandidats <- emplois.ec %>% 
  filter(Périmètre.ID == "Ensemble") %>%
  mutate(Concours.NonPourvus.MCF = Concours.Postes.MCF - Concours.Recrutés.MCF) %>%
  pivot_longer(cols=c(Concours.Postes.MCF,Concours.Candidats.MCF), names_to = "Série", values_to = "Valeur") %>%
  mutate(Série = factor(Série, 
                        levels=c("Concours.Postes.MCF","Concours.Candidats.MCF"),
                        labels=c("Postes MCF","Candidats") )) %>%
  filter(Année > 2014, Année < 2024) %>%
  mutate(Valeur100 = Valeur/first(Valeur)*100, .by=Série) %>%
  ggplot(aes(x=Année, y=Valeur100, color=Série)) +
  geom_line(linewidth = 1) + geom_point(shape = 21, stroke = 1.5, size = 1.5, fill="white") +
  scale_x_continuous(breaks = scales::pretty_breaks(12)) +
  scale_color_brewer(palette="Set1", direction = -1) +
  labs(title="Concours MCF : Postes ouverts et nombre de candidats", 
       y = "Valeur 100 en 2015") 
plot_postesMCFvsCandidats
```

## plot_postesMCFnonPourvus

```{r, out.width="90%", fig.align="center"}
plot_postesMCFnonPourvus <- emplois.ec %>% 
    filter(Année > 2014, Année < 2024) %>%
  filter(Périmètre.ID %in% c("Ensemble","ST","LLASHS","DEG","27")) %>%
  mutate(Périmètre.label=factor(Périmètre.label, 
        levels=c("Ensemble","Sciences et techniques","Lettres et sciences humaines","Droit éco gestion","Informatique"))) %>%
  ggplot(aes(x=Année, y=kpi.MCF.TauxPostesNonPourvus, color=Périmètre.label)) +
  geom_line(linewidth = 1) + geom_point(shape = 21, stroke = 1.5, size = 1.5, fill="white") +
  scale_x_continuous(breaks = scales::pretty_breaks(12)) +
  scale_y_continuous(labels = scales::percent) +
  labs(title="Concours MCF : Taux de postes non pourvus", 
       y = "Taux de postes non pourvus") 
plot_postesMCFnonPourvus
```


## plot_BMO

```{r}
bmo <- read.csv2("Besoins en Main-d’Œuvre_Top 10 en nombre de projets de recrutement non saisonniers en 2024 par Métie.csv", skip = 3) %>%
  mutate(Projets.de.recrutement.non.saisonniers = as.numeric(gsub(" ","",Projets.de.recrutement.non.saisonniers))) %>%
  mutate(Voie = factor(case_when(
    startsWith(Métier,"Inf") ~ "Formation supérieure de santé",
    TRUE ~ "Formation professionnelle"
  ),
  levels = c("Formation professionnelle","Formation supérieure de santé","Formation universitaire","Formation d'ingénieur"),
  labels = c("Formation professionnelle","Formation supérieure de santé","Formation universitaire","Formation d'ingénieur")))
  

plot_bmo <- bmo  %>%
  ggplot(aes(x=Projets.de.recrutement.non.saisonniers,
             y=reorder(str_wrap(Métier,40),Projets.de.recrutement.non.saisonniers),
             fill = Voie)) +
  geom_col(show.legend = TRUE) +
  scale_x_continuous(labels = scales::number) +
  scale_fill_brewer(palette = "Dark2", drop=FALSE,guide = guide_legend(nrow = 2)) +
  labs(title = "Top 10 des métiers les plus demandés en 2024",
       x = "Nombre de projets de recrutement non saisonniers",
       y = NULL) +
  theme(legend.position = "top")

plot_bmo
```

### Massification (1960-2000) : Investissements massifs dans l'éducation

```{r diplome, cache = TRUE}
load("emploi.RData")

#Graphique pour connaître le niveau de diplome des jeunes selon l'année civile 
plot_diplome <- function(agemin = 15, agemax = 30, anneemin = 1971, anneemax = 2020) {
  emploi %>%
    filter(Age > agemin, Age < agemax, Annee >= anneemin, Annee <= anneemax) %>%
    mutate(Diplome = factor(Diplome,
                            levels=c("Bac+5","Bac+3","Bac+2","Bac","CAP-BEP","DNB","Aucun"))) %>%
    mutate(Diplome = fct_recode(Diplome,
                                Tertiaire = "Bac+5",
                                Tertiaire = "Bac+3",
                                Tertiaire = "Bac+2",
                                Secondaire = "Bac",
                                Secondaire = "CAP-BEP",
                                Secondaire = "DNB",
                                Primaire = "Aucun") ) %>% 
    group_by(Annee,Diplome) %>%
    summarise(Population = sum(Population)/1e6) %>%
    ungroup() %>%
    na.omit() %>%
    filter(Annee != 1973) %>%
    complete(Annee,Diplome,fill=list(Population = 0)) %>%
    ggplot(aes(x=Annee,y=Population,fill=Diplome,group=Diplome)) +
    geom_area(color="white", alpha=0.6, position = "fill") + 
    #geom_vline(xintercept = c(1994,2007)) +
    xlab("Année")
}

plot_stratification_continu <- plot_diplome(agemin = 25, agemax = 30) +
  scale_y_continuous(labels = scales::percent, name = "Part des jeunes de 25 à 30 ans") +
  cpesr_cap(authors = "Camille Schumacher et Julien Gossa", source = "Enquête emploi INSEE")

plot_stratification_continu
```


### Massification (1960-2000) : Investissements massifs dans l'éducation


```{r diplome.2, cache = TRUE}
#Graphique pour connaître le niveau de diplome des jeunes selon l'année civile 
plot_categorie_ponctuel <- function(categorie="Activite", agemin = 15, agemax = 30, anneemin = 1971, anneemax = 2020, fill=TRUE, keep_na=FALSE) {
  if(fill) {
    position_geom <- "fill"
    label_scale <- ~ scales::percent(abs(.x))
    title_y <- "Pourcentage de la population 25-30"
  } else {
    position_geom <- "stack"
    label_scale <- function(x) x/1e6
    title_y <- "Effectif (millions)"
  }
  
  emploi %>%
    filter(Age >= agemin, Age <= agemax, Annee %in% c(anneemin,anneemax)) %>%
        mutate(Diplome = factor(Diplome,
                            levels=c("Bac+5","Bac+3","Bac+2","Bac","CAP-BEP","DNB","Aucun"))) %>%
    mutate(Diplome = fct_recode(Diplome,
                                Tertiaire = "Bac+5",
                                Tertiaire = "Bac+3",
                                Tertiaire = "Bac+2",
                                Secondaire = "Bac",
                                Secondaire = "CAP-BEP",
                                Secondaire = "DNB",
                                Primaire = "Aucun") ) %>% 
    mutate(Diplome = fct_rev(Diplome)) %>%
    group_by(Annee, Sexe, Categorie=!!as.name(categorie)) %>%
    summarise(Population = sum(Population, na.rm=TRUE)) %>%
    { if (!keep_na) na.omit(.) else . } %>%
    { if (fill) group_by(.,Annee) %>% mutate(Population = Population / sum(Population)) else . } %>%
    ungroup() %>%
    complete(Annee, Categorie, fill = list(Population = 0)) %>% 
    mutate(Population = ifelse(Sexe == "H",-Population,Population)) %>%
    ggplot(aes(y=Categorie,x=Population,fill=Sexe, group=Sexe)) +
    geom_col(alpha=0.8, color="white",width = 1) + 
    expand_limits(x=c(-0.28,0.28)) +
    facet_grid(.~Annee) +
    scale_x_continuous(labels = label_scale) +
    scale_fill_brewer(palette = "Set2") +
    labs (y = "Diplôme", x = title_y, caption = "Source :Enquête Emploi (1971 - 2020)")
}


plot_stratification_ponctuel <- plot_categorie_ponctuel(categorie="Diplome", agemin = 25, agemax = 30) +
  #scale_y_continuous(labels = scales::percent, name = "Part des jeunes de 25 à 30 ans") +
  cpesr_cap(authors = "Camille Schumacher et Julien Gossa", source = "Enquête emploi INSEE")

plot_stratification_ponctuel
```


## Emplois

```{r}
emploiec.path <- "~/repos/cpesr/emploiESR"
cnu.sections <- read.csv2(paste0(emploiec.path,"/data/cpesr-cnu-sections.csv"))
emploisEC <- read.csv2(paste0(emploiec.path,"/data/cpesr-emplois-ec.csv"), stringsAsFactors = TRUE)
source(paste0(emploiec.path,"/R/emploiEC-metriques.R"))
source(paste0(emploiec.path,"/R/emploiEC-plot-series.R"))
source(paste0(emploiec.path,"/R/emploiEC-plot-metrique.R"))
source(paste0(emploiec.path,"/R/emploiEC-plot-contexte.R"))
année.ref <- emploisEC %>% filter(Données == "Consolidées") %>% tail(n = 1) %>% pull(Année)

plot_emploiMCF <- plot_emplois_long() + ggtitle("Postes ouverts pour entrer dans la carrière d'enseignant-chercheur titulaire")

plot_emploiMCF
```

## R&D

```{r}
rds <- read.csv("rd_spendings.csv")

plot_rd <- rds %>%
  arrange(desc(rd_spending_usd_billion_2022)) %>%
  mutate(country = factor(country, levels = unique(country))) %>%
  head(11) %>%
  ggplot(aes(x=rd_spending_usd_billion_2022,y=reorder(company,rd_spending_usd_billion_2022),fill=country)) +
  geom_col() +
  geom_text(aes(label = paste(round(rd_spending_usd_billion_2022), "Md$")), hjust = 0, nudge_x = 0.5) +
  scale_fill_brewer(palette = "Accent", direction=-1) +
  expand_limits(x=77) +
  coord_cartesian(clip = "off") +
  labs(
    title = "Top 10 des dépenses R&D des entreprises (2022) et MIRES (2026)",
    x = "Dépense de R&D (Md$)",
    y = NULL, 
    fill = "Pays") +
  theme(legend.position = "inside", legend.position.inside = c(0.85, 0.3))

plot_rd
```

Source : https://en.wikipedia.org/wiki/List_of_companies_by_research_and_development_spending

```{r}
labels <- rd %>%
  arrange(annual_year) %>%
  group_by(ticker) %>%
  slice_tail(n=1)

plot_rd_long <- rd %>%
  ggplot(aes(x=annual_year, y=annual_billion, color=country, group=ticker)) + 
  geom_line(size = 1) +
  ggrepel::geom_text_repel(data=labels, aes(label=company_name), color="black", direction="y", nudge_x = 0.2, hjust=0) +
  expand_limits(x=2027.5) +
  scale_color_brewer(palette="Set1") +
  labs(
    title = "Dépenses R&D des GAFAMS et des administrations françaises",
    subtitle = "DIRD = dépense intérieure de R&D en France - DIRDA = DIRD des administrations",
    x = "Année",
    y = "Dépense de R&D (Md$)",
    color = NULL
  ) +
  theme(legend.position = "None")

plot_rd_long
```

## Budget RD

```{r, results='asis'}
plot_dird <- oecd_plot("G") 
plot_dird
```

```{r, results='asis'}
plot_gafam <- oecd_corp_plot("H", pays="FRA")
plot_gafam
```

## NEET

```{r, results='asis'}
library(insee)

neet_insee <- get_insee_idbank("010752067") |>
  transmute(
    date = DATE,
    neet = OBS_VALUE
  ) |>
  arrange(date)


plot_neet_insee <- neet_insee %>% ggplot(aes(x=date,y=neet)) + 
  geom_line(color="orange", size=1) +
  geom_vline(xintercept = as.Date("2019-09-01"), linewidth=0.4) +
  annotate("text", x=as.Date("2019-09-01"), y=10, label="Loi Liberté de\nchoisir son avenir pro.",
           vjust = 0.5, hjust = 0, angle = 90) +
  scale_x_date(breaks=scales::pretty_breaks(n=20), name=NULL) +
  scale_y_continuous(labels=~paste(.x,"%"), breaks = scales::pretty_breaks(n=5),
                     name="Taux de NEET (15-29 ans)") +
  labs(
    title = "Taux de NEET trimestriel, en France, 15-29 ans",
    subtitle = "Jeunes ni en emploi ni en formation",
    caption = "Source : INSEE"
  )
plot_neet_insee  
```


```{r, results='asis'}
library(eurostat)

neet_eu <- get_eurostat(
  "edat_lfse_20",
  time_format = "num"
) |>
  filter(
    freq == "A",
    sex == "T",
    age == "Y15-29",
    training == "NO_FE_NO_NFE",
    wstatus == "NEMP",
    unit == "PC",
    geo %in% c("FR", "EU27_2020")
  ) |>
  transmute(
    year = TIME_PERIOD,
    geo = recode(
      geo,
      "FR" = "France",
      "EU27_2020" = "UE-27"
    ),
    neet = values
  )

plot_neet_eu <- neet_eu %>% 
  #filter(year >= 2012) %>%
  mutate(geo = fct_rev(geo)) %>%
  ggplot(aes(x=year,y=neet,color=geo)) + 
  geom_line(size=1) +
  geom_vline(xintercept = 2019, size=0.4) +
  annotate("text", x=2019, y=10, label="Loi Liberté de\nchoisir son\navenir pro.",
           vjust = 0.3, hjust = 0, angle = 90) +
  scale_x_continuous(breaks=scales::pretty_breaks(n=11), name=NULL) +
  scale_y_continuous(labels=~paste(.x,"%"), breaks = scales::pretty_breaks(n=5),
                     name="Taux de NEET (15-29 ans)") +
  scale_color_manual(values=c("grey","orange"), name="Zone") +
  labs(
    title = "Taux de NEET annuel, en France et UE27, 15-29 ans",
    subtitle = "Jeunes ni en emploi ni en formation",
    caption = "Source : Eurostat"
  )  
plot_neet_eu
```

## Naissances

```{r, results='asis'}
library(insee)

naissances <- get_insee_idbank("001641590",startPeriod = 1900) |>
  transmute(
    year = as.integer(TIME_PERIOD),
    naissances = OBS_VALUE
  ) |>
  arrange(year)

maxyear <- max(naissances$year)
naisbac <-  maxyear - 18
naismin <- min(naissances$naissances)

plot_naissances <- naissances %>% 
  ggplot(aes(x=year,y=naissances)) +
  geom_line(color="darkorchid", linewidth=1) +
  geom_vline(xintercept = naisbac, size=0.4) +
  annotate("text", x=naisbac, y=naismin, label=paste("Naissance des\nnéo-bachelier⋅res", maxyear),
           vjust = 0.5, hjust = 0, angle = 90) +
  scale_x_continuous(breaks=scales::pretty_breaks(n=20), name=NULL) +
  scale_y_continuous(labels=~paste(.x/1e3,"k"), breaks = scales::pretty_breaks(n=10),
                     name="Naissances vivantes") +
  labs(
    title = "Nombre de naissances vivantes en France",
    caption = "Source : INSEE"
  )
plot_naissances
```

```{r, results='asis'}
refyear <- maxyear+1

ages_education <- tribble(
  ~niveau,       ~age_entree,
  "Maternelle",           3,
  "Primaire",             6,
  "Collège",             11,
  "Lycée",               15,
  "Licence",             18,
  "Master",              21,
  "Doctorat",            23,
  "Post-éducation",      26,
  "Retraite",            64,
  "Fin",                 refyear-1955
) %>%
  mutate(niveau=factor(niveau,levels=niveau)) %>%
  mutate(xmax = refyear - age_entree) %>%
  mutate(xmin = lead(xmax))

plot_naissances_niveaux <- naissances %>% 
  ggplot() +
  geom_rect(data=ages_education,aes(xmin=xmin,xmax=xmax,ymin=-Inf,ymax=Inf,fill=niveau), alpha=0.4) +
  geom_text(data=ages_education,aes(x=(xmin+xmax)/2,y=naismin-50e3,label=niveau),
            vjust = 0.5, hjust = 0, angle = 90) +
  geom_line(aes(x=year,y=naissances), color="darkorchid", linewidth=1) +
  scale_x_continuous(breaks=scales::pretty_breaks(n=20), name="Année de naissance") +
  scale_y_continuous(labels=~paste(.x/1e3,"k"), breaks = scales::pretty_breaks(n=10),
                     name="Naissances vivantes") +
  scale_fill_brewer(palette = "RdBu") +
  labs(
    title = "Nombre de naissances vivantes et niveaux éducatifs standards en 2026",
    caption = "Source : INSEE"
  ) +
  theme(legend.position = "None")

plot_naissances_niveaux
```

## Save

```{r}
save(list = c(
    ls(pattern = "^plot_"),
    "naismin",
    "levels_gdcnu"),
  file = "plots.RData")
```
